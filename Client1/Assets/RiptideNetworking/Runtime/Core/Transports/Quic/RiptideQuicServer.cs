...
namespace Riptide.Transports.Quic
{
    ...
    public class RiptideQuicServer : RiptideQuicPeer, IServer
    {
        /// <inheritdoc/>
        ...
        /// <inheritdoc/>
        public RiptideQuicServer(SocketMode mode = SocketMode.Both, int socketBufferSize = DefaultSocketBufferSize) : base(mode, socketBufferSize) { }
        ...
        private bool HandleConnectionAttempt(RiptideQuicConnection connection)
        {
        ...
        {
            if (connection is RiptideQuicConnection udpConnection)
        ...
        {
            if ((MessageHeader)dataBuffer[0] == MessageHeader.Connect && !HandleConnectionAttempt(new RiptideQuicConnection(fromEndPoint, this)))
                return;
