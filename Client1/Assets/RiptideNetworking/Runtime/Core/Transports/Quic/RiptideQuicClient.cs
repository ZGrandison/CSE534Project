...
namespace Riptide.Transports.Quic
{
    ...
    public class RiptideQuicClient : RiptideQuicPeer, IClient
    {
        /// <inheritdoc/>
        ...
        private RiptideQuicConnection udpConnection;

        /// <inheritdoc/>
        public RiptideQuicClient(SocketMode mode = SocketMode.Both, int socketBufferSize = DefaultSocketBufferSize) : base(mode, socketBufferSize) { }
        ...
        public bool Connect(string hostAddress, out Connection connection, out string connectError)
        {
            ...
            connection = udpConnection = new RiptideQuicConnection(new IPEndPoint(mode == SocketMode.IPv4Only ? ip : ip.MapToIPv6(), port), this);
