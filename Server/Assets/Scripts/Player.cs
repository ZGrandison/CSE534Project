...
namespace Riptide.Demos.DedicatedServer
{
    ...
    public class Player : MonoBehaviour
    {
        ...
        private void FixedUpdate()
        {
            ...
            if (Inputs[5])
            {
                /*GameObject bullet = Instantiate(NetworkManager.Singleton.BulletPrefab, muzzle.transform.position, Quaternion.identity);
                bullet.transform.forward = muzzle.transform.forward;*/
                CreateBullet(gunMuzzle.transform.position, gunMuzzle.transform.forward);
            }
